...
#define __COMPILING_NS_OBJMESH_CPP__
#include "ObjMesh.h"
#undef __COMPILING_NS_OBJMESH_CPP__
...
ObjMesh::ObjMesh(Scene* scene, const string& name)
: Geometry(scene),
m_name(name),
m_faceCount(0),
m_vertexCount(0)
...
int ObjMesh::GetVertexCount() const
{
    if (m_vertexCount != 0)
...
int ObjMesh::GetTriangleIndexCount() const
{
    if (m_faceCount != 0)
...
void ObjMesh::GenerateVertices(vector<float>& floats, unsigned char flags) const
{
...
void ObjMesh::GenerateTriangleIndices(vector<unsigned short>& indices) const
{
